...
public:
	...
	bool Init() {
		...
		if (!sts) {
			MessageBox(NULL, L"CreateBuffer(constant buffer world) error", L"Error", MB_OK);
			return false;
		}
		...
		if (!sts) {
			MessageBox(NULL, L"CreateBuffer(constant buffer view) error", L"Error", MB_OK);
			return false;
		}
		...
		if (!sts) {
			MessageBox(NULL, L"CreateBuffer(constant buffer projection) error", L"Error", MB_OK);
			return false;
		}
		return true;
	}
