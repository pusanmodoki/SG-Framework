...
bool CDirectXGraphics::Init(HWND hWnd, unsigned int Width, unsigned int Height, bool fullscreen)
{
 ...
 //   createDeviceFlags |= D3D11_CREATE_DEVICE_DEBUG;
	...
	hr = m_lpDevice->CreateDepthStencilState(&depthStencilDesc, &m_depthStencilEnableState);
	if (FAILED(hr)){
		return false;
	}

	// デバイスコンテキストへセット
	m_lpImmediateContext->OMSetDepthStencilState(m_depthStencilEnableState, 1);
...
void	CDirectXGraphics::Exit()
{
	...
	if (m_depthStencilEnableState) {
		m_depthStencilEnableState->Release();
		m_depthStencilEnableState = 0;
	}
